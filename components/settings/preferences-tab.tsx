...
export function PreferencesTab() {
  ...
  return (
    <div className="pb-safe pb-24">
      ...
      {/* ── Tuần bắt đầu từ — vẫn ẩn: weekStartDay chưa kết nối vào logic
          (7 call-site đang hardcode 'monday', xem DESIGN_NOTES.local.md) ─── */}
      {false && (
        <Row className="flex items-center justify-between">
      ...
      <Section>
        ...
        <Row className="flex items-center justify-between gap-4">
          <div className="flex-1">
            <p className="text-sm font-medium text-foreground">Xem theo kỳ lương</p>
            <p className="text-xs text-muted-foreground mt-0.5">
              {settings?.salaryDay ? 'Thu/chi tính từ ngày nhận lương thay vì đầu tháng' : 'Đặt ngày nhận lương trước'}
            </p>
          </div>
          <ToggleSwitch
            checked={settings?.cycleModeEnabled ?? false}
            onChange={v => save('cycleMode', () => updateSettings(user.uid, { cycleModeEnabled: v }))}
            disabled={saving === 'cycleMode' || !settings?.salaryDay}
          />
        </Row>
      </Section>
